...
router.use(requireRole("influencer", "employee", "admin"));
...
> {
  ...
  return withTx(async (client) => {
    ...
    const tc = Math.max(1, Math.min(100, Number(ord.task_count) >= 1 ? Number(ord.task_count) : 1));
...
router.get("/market-orders", (req: AuthRequest, res: Response) => {
  ...
  (async () => {
    ...
    let sql = `SELECT mo.id, mo.order_no, mo.title, mo.tier, mo.publish_method, mo.voice_link, mo.voice_note, mo.tiktok_link, mo.product_images, mo.sku_codes, mo.sku_images,
                      ...
                      mo.creator_reward_points AS reward_points, (mo.creator_reward_points * GREATEST(COALESCE(mo.task_count, 1), 1))::integer AS reward_points_total, mo.status, mo.created_at,
