  ...
  (async () => {
    ...
    let sql = `
      ...
      SELECT m.id, m.name, m.photos, m.intro, m.cloud_link, m.status, m.pending_status,
             ...
             m.tiktok_followers_text, m.tiktok_sales_text, m.tiktok_live_sales_text, m.tiktok_gmv_text, m.sellable_product_types, m.skills_text,
...
router.post("/", (req: AuthRequest, res: Response) => {
  ...
  const tiktokLiveSales = normalizeOptionalModelField(req.body?.tiktok_live_sales_text, MAX_TIKTOK_METRIC_LEN);
  const tiktokGmv = normalizeOptionalModelField(req.body?.tiktok_gmv_text, MAX_TIKTOK_METRIC_LEN);
  ...
  (async () => {
    ...
    const { rows } = await query<{ id: number }>(
      ...
      `INSERT INTO model_profiles (name, photos, intro, cloud_link, status, pending_status,
          ...
          tiktok_followers_text, tiktok_sales_text, tiktok_live_sales_text, tiktok_gmv_text, sellable_product_types, skills_text, created_by, updated_by)
