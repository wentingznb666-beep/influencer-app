  ...
  (async () => {
    ...
    let sql = `
      ...
      SELECT mo.id, mo.order_no, mo.title,
             ...
             mo.reward_points AS client_pay_unit,

             (mo.reward_points * GREATEST(COALESCE(mo.task_count, 1), 1))::integer AS client_pay_total,

             CASE WHEN $1 = 'employee' THEN NULL ELSE mo.creator_reward_points END AS creator_reward_unit,

             CASE WHEN $1 = 'employee' THEN NULL ELSE (mo.creator_reward_points * GREATEST(COALESCE(mo.task_count, 1), 1))::integer END AS creator_reward_total,
