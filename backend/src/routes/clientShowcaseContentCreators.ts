...
router.get("/", (req: AuthRequest, res: Response) => {
  ...
  (async () => {
    ...
    let idx = 2;
    let sql = `
      SELECT s.id, s.name, s.intro, s.photos, s.shoot_types_text, s.skills_text, s.video_url, s.tiktok_sales, s.status, s.updated_at,
        ...
        LEFT JOIN client_showcase_creator_favorites c
          ON c.showcase_content_creator_id = s.id AND c.client_id = $1 AND c.is_deleted = 0
    ...
    console.error("client showcase content creators list error:", e);
...
router.get("/my", (req: AuthRequest, res: Response) => {
  ...
  (async () => {
    const { rows } = await query(
      `SELECT s.id, s.name, s.intro, s.photos, s.shoot_types_text, s.skills_text, s.video_url, s.tiktok_sales, s.updated_at
    ...
    console.error("client my showcase content creators error:", e);
    res.status(500).json({ error: "INTERNAL_SERVER_ERROR", message: getUserFriendlyError(e) });
  });
});

/**
 * PUT /api/client/showcase-content-creators/:id/selection
 * 预约或取消预约 Content Creator。
 */
router.put("/:id/selection", (req: AuthRequest, res: Response) => {
  ...
  (async () => {
    const row = await query<{ id: number }>(
      "SELECT id FROM showcase_content_creators WHERE id = $1 AND is_deleted = 0 AND status = 'enabled'",
      [showcaseId]
    );
    if (!row.rows[0]) {
    ...
    if (selected) {
      await query(
        `INSERT INTO client_showcase_creator_favorites (client_id, showcase_content_creator_id, is_deleted, created_at, updated_at)
         VALUES ($1, $2, 0, now(), now())
         ON CONFLICT (client_id, showcase_content_creator_id)
         DO UPDATE SET is_deleted = 0, updated_at = now()`,
        [clientId, showcaseId]
      );
    } else {
      await query(
        "UPDATE client_showcase_creator_favorites SET is_deleted = 1, updated_at = now() WHERE client_id = $1 AND showcase_content_creator_id = $2 AND is_deleted = 0",
        [clientId, showcaseId]
      );
    }
    res.json({ ok: true });
  })().catch((e) => {
    console.error("client showcase creator selection error:", e);
    res.status(500).json({ error: "INTERNAL_SERVER_ERROR", message: getUserFriendlyError(e) });
  });
});
