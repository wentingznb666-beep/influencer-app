...
clientRouter.post("/connections", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!influencer_id || !category) return res.status(400).json({ error: "MISSING" });
    ...
    const { rows } = await query(
      ...
      [req.user!.userId, influencer_id, influencer_profile_id || null, category, grade || null, brief || null, budget || null, start, end]
    );
    await sendMsg(influencer_id, "connection_invite", "新的建联邀请", `商家向你发起了建联邀请，类目：${category}`, "/influencer/connections?tab=pending");
...
clientRouter.post("/connections/:id/renew", async (req: AuthRequest, res: Response) => {
  try {
    ...
    await sendMsg(existing.rows[0].influencer_id, "connection_invite", "建联已续约", "建联已续约30天", "/influencer/connections?tab=active");
...
clientRouter.post("/connection-orders", async (req: AuthRequest, res: Response) => {
  try {
    ...
    await sendMsg(influencer_id, "connection_order", "新的定向派单", `商家向你派发了新订单：${title}`, `/influencer/connection-orders`);
...
clientRouter.post("/connection-orders/:id/review", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (action === "approve") {
      ...
      await sendMsg(order.influencer_id, "connection_order", "订单验收通过", `订单 ${order.order_no} 已验收通过`, `/influencer/connection-orders`);
      ...
      await sendMsg(order.influencer_id, "connection_order", "订单需要修改", `订单 ${order.order_no} 被驳回：${review_note}`, `/influencer/connection-orders`);
    }
    res.json({ ok: true });
  } catch (e: any) { res.status(500).json({ error: "INTERNAL_ERROR", message: e.message }); }
});
...
clientRouter.post("/connection-orders/:id/confirm-payment", async (req: AuthRequest, res: Response) => {
  try {
    ...
    await sendMsg(order.rows[0].influencer_id, "connection_payment", "商家已付款", `订单 ${order.rows[0].order_no} 商家已确认付款`, `/influencer/connection-orders`);
    res.json({ ok: true });
  } catch (e: any) { res.status(500).json({ error: "INTERNAL_ERROR", message: e.message }); }
});
...
influencerRouter.patch("/connections/:id", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (action === "accept") {
      ...
      await sendMsg(conn.client_id, "connection_invite", "建联已接受", "达人已接受你的建联邀请", "/client/connections?tab=active");
    } else if (action === "reject") {
      await query("UPDATE influencer_connections SET status = 'rejected', updated_at = now() WHERE id = $1", [req.params.id]);
      await sendMsg(conn.client_id, "connection_invite", "建联已拒绝", "达人已拒绝你的建联邀请", "/client/connections");
    }
    res.json({ ok: true });
  } catch (e: any) { res.status(500).json({ error: "INTERNAL_ERROR", message: e.message }); }
});
...
influencerRouter.patch("/connection-orders/:id/respond", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (action === "accept") {
      ...
      await sendMsg(order.client_id, "connection_order", "达人已接受派单", `订单 ${order.order_no} 达人已接受`, `/client/connection-orders`);
    } else if (action === "reject") {
      ...
      await sendMsg(order.client_id, "connection_order", "达人已拒绝派单", `订单 ${order.order_no} 达人已拒绝：${reject_reason}`, `/client/connection-orders`);
    }
    res.json({ ok: true });
  } catch (e: any) { res.status(500).json({ error: "INTERNAL_ERROR", message: e.message }); }
});
...
influencerRouter.post("/connection-orders/:id/submit", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (order.rows[0]) await sendMsg(order.rows[0].client_id, "connection_order", "达人已提交作品", `订单 ${order.rows[0].order_no} 达人已提交作品`, `/client/connection-orders`);
    res.json({ ok: true });
  } catch (e: any) { res.status(500).json({ error: "INTERNAL_ERROR", message: e.message }); }
});
...
influencerRouter.post("/connection-orders/:id/revise", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const order = await query("SELECT * FROM connection_orders WHERE id = $1", [req.params.id]);
    if (order.rows[0]) await sendMsg(order.rows[0].client_id, "connection_order", "达人已修改重提", `订单 ${order.rows[0].order_no} 达人已修改重提`, `/client/connection-orders`);
    res.json({ ok: true });
  } catch (e: any) { res.status(500).json({ error: "INTERNAL_ERROR", message: e.message }); }
});
