...
- **角色与端口**：对应 PRD 第 2 节（管理员端、达人端、客户端）。
...
- [x] 设计并实现用户表、角色表（管理员/客户/达人）（优先级：高）（模块：基础）
...
- [x] 前端：按角色跳转不同入口或路由（管理员/客户/达人）（优先级：高）（模块：基础）
...
- [x] 设计并实现客户任务/需求表（产品信息、目标平台、预算、是否露脸）（优先级：高）（模块：客户端）
- [x] 实现客户合作信息与样品寄送记录表及 API（优先级：高）（模块：客户端）
- [x] 实现订单跟踪系统 API（状态、时间节点、关联达人/任务）（优先级：高）（模块：客户端）
- [x] 实现客户查看达人发布作品列表 API（优先级：高）（模块：客户端）
- [x] 实现视频单量、播放量等 API 接口数据（为后续投流码/报表预留）（优先级：中）（模块：客户端）
- [x] 客户端充值得积分接口与流水（优先级：中）（模块：客户端）

#### 客户端（前端）

- [x] 合作意向与任务需求提交页（产品、平台、预算、是否露脸）（优先级：高）（模块：客户端）
- [x] 样品寄送与订单跟踪页（列表、状态、时间线）（优先级：高）（模块：客户端）
- [x] 达人已发布作品列表与基础数据展示（优先级：高）（模块：客户端）
- [x] 视频下载（露脸/讲解）说明与按组下载、防关联说明（优先级：中）（模块：客户端）
- [x] 积分充值页与积分余额展示（优先级：中）（模块：客户端）
...
|----------------|------|----------|------------------------|
| 客户端 | 待开发 | 阶段 3 | 客户任务/样品/订单跟踪、作品列表、积分充值；前端合作意向、订单跟踪、作品展示 |
