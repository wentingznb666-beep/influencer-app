...
export default function PointsPage() {
  ...
  return (
    <div>
      <h2 className="xt-inf-page-title">积分与收益</h2>
      <p className="xt-inf-lead">1 积分 = 1 泰铢，按周结算；流水为最近记录。</p>
        ...
        <>
          <div style={{ display: "flex", gap: 16, marginBottom: 24, flexWrap: "wrap" }}>
            <div className="xt-inf-card" style={{ padding: 18, minWidth: 168, flex: "1 1 160px" }}>
              <div style={{ fontSize: 13, color: "var(--xt-text-muted)" }}>总积分</div>
              <div style={{ fontSize: 28, fontWeight: 800, color: "var(--xt-accent)", marginTop: 6 }}>{balance}</div>
            </div>
            <div className="xt-inf-card" style={{ padding: 18, minWidth: 168, flex: "1 1 160px" }}>
              <div style={{ fontSize: 13, color: "var(--xt-text-muted)" }}>本周已获得</div>
              <div style={{ fontSize: 28, fontWeight: 800, color: "var(--xt-primary)", marginTop: 6 }}>{weekPending}</div>
            </div>
          </div>
          <h3 style={{ fontSize: 16, marginBottom: 10 }}>最近流水</h3>
          <div style={{ overflowX: "auto" }} className="xt-inf-card">
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 520 }}>
              <thead>
                <tr style={{ background: "#f8fafc" }}>
                  <th style={{ padding: 12, textAlign: "left", fontWeight: 700 }}>时间</th>
                  <th style={{ padding: 12, textAlign: "left", fontWeight: 700 }}>类型</th>
                  <th style={{ padding: 12, textAlign: "right", fontWeight: 700 }}>积分</th>
                </tr>
              </thead>
              <tbody>
                {ledger.map((l) => (
                  <tr key={l.id} style={{ borderTop: "1px solid var(--xt-border)" }}>
                    ...
                    <td style={{ padding: 12 }}>{ledgerTypeLabel(l.type)}</td>
                    <td
                      style={{
                        padding: 12,
                        textAlign: "right",
