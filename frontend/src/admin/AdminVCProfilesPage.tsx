        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", background: "#fff", fontSize: 12 }}>
            <thead><tr style={{ background: "#f8fafc" }}>
                ...
                {["ID","编号","来源","粉丝","类目","等级","GMV","挂车","件数","直播","直播销售","类型","状态","操作"].map(h => <th key={h} style={{ padding: "6px 8px", textAlign: "left", borderBottom: "1px solid #e2e8f0", fontWeight: 700, whiteSpace: "nowrap" }}>{t(h)}</th>)}
              </tr></thead>
            <tbody>
              {list.map((p: any) => (
                <tr key={p.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  <td style={{padding:"6px 4px"}}><input type="checkbox" checked={selected.has(p.id)} onChange={()=>toggleSelect(p.id)} style={{width:14,height:14}} /></td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.id}</td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.influencer_code}</td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.source==="contact_us"?t("我方联系"):t("达人联系")}</td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.followers||"-"}</td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.category}</td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.grade||t("未达标")}</td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.gmv_sales||"-"}</td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.monthly_cart_videos||"-"}</td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.units_sold||"-"}</td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.can_live?t("是"):t("否")}</td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.live_sales||"-"}</td><td style={{ padding: "6px 8px", fontSize: 12 }}>{p.user_id ? <span style={{background:"#dbeafe",color:"#1d4ed8",padding:"1px 8px",borderRadius:999,fontSize:11,fontWeight:600}}>👤 自主</span> : <span style={{background:"#f1f5f9",color:"#64748b",padding:"1px 8px",borderRadius:999,fontSize:11,fontWeight:600}}>🛠 托管</span>}</td>
