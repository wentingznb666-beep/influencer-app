...
import { getAdminInfluencerPermissions } from "../matchingApi";
...
export default function UsersPage() {
  ...
  const [merchantExtraMap, setMerchantExtraMap] = useState<Record<number, MerchantExtra>>({});
  const [influencerExtraMap, setInfluencerExtraMap] = useState<Record<number, InfluencerExtra>>({});
  ...
  const load = async () => {
    ...
    try {
      ...
      const [usersData, merchantData, influencerPermissionData] = await Promise.all([
        api.getUsers({ keyword: keyword.trim(), role: roleFilter, disabled: disabledFilter }),
        api.getAdminMerchantMembers().catch(() => ({ list: [] })),
        getAdminInfluencerPermissions().catch(() => ({ list: [] })),
      ]);

      setList(((usersData.list || []) as UserItem[]).map((item) => ({
        ...item,
        username: normalizeAccountText(item.username),
        display_name: item.display_name ? normalizeAccountText(item.display_name) : item.display_name,
      })));
      ...
      for (const m of (merchantData.list || []) as any[]) {
        ...
        merchantMap[id] = {
          ...
          member_expire_time: m.member_expire_time ?? null,
          deposit_status: m.deposit_status ?? null,
        };
      }
      ...
      for (const inf of (influencerPermissionData.list || []) as any[]) {
        ...
        influencerMap[id] = {
          influencer_status: inf.influencer_status ?? null,
          bank_summary: String(bankSummary),
        };
      }
      setInfluencerExtraMap(influencerMap);
                  ...
                  <td style={{ padding: 10, borderBottom: "1px solid #f1f1f1", fontSize: 12, color: "#475569" }}>
                    {item.role === "client" ? (
                      <>
                        ...
                        <br />
                        到期：{merchantExtraMap[item.id]?.member_expire_time || "-"}
                        <br />
                        保证金状态：{merchantExtraMap[item.id]?.deposit_status || "none"}
                      </>
                    ) : item.role === "influencer" ? (
                      <>
                        撮合权限：{influencerExtraMap[item.id]?.influencer_status || "unapplied"}
                        <br />
                        收款信息：{influencerExtraMap[item.id]?.bank_summary || "未填写"}
                      </>
                    ) : (
                      "-"
                    )}
