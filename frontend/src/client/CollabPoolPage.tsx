...
export default function CollabPoolPage() {
  ...
  return (
    <div className="xt-card" style={{ padding: compactPx(20) }}>
      ...
      <div className="xt-form" style={{ maxWidth: compactPx(640), marginBottom: compactPx(16) }}>
      ...
      {msg && <p style={{ padding: "8px 12px", borderRadius: compactPx(8), background: msg.includes("成功") ? "#dcfce7" : "#fee2e2", color: msg.includes("成功") ? "#15803d" : "#b91c1c", fontWeight: 600, fontSize: compactPx(13) }}>{msg}</p>}
      {loading ? <p>{t("加载中…")}</p> : null}
      {!loading && list.length === 0 ? <p>{t("暂无需求")}</p> : null}
      <ul style={{ listStyle: "none", padding: 0, display: "grid", gap: compactPx(12) }}>
        {list.map((it) => {
          ...
          return (
            <li key={it.id} style={{ background: "var(--xt-bg)", borderRadius: compactPx(12), padding: compactPx(16), border: "1px solid var(--xt-border)" }}>
              ...
              <button type="button" className="xt-btn-outline" style={{ marginTop: compactPx(8) }} onClick={() => void consult(it.id)}>{t("咨询")}</button>
              <button type="button" className="xt-accent-btn" style={{ marginLeft: compactPx(8), marginTop: compactPx(8) }} disabled={it.my_apply_status === "pending" || it.my_apply_status === "selected"} onClick={() => void apply(it.id)}>
