 ...
 * ????????????? group ??????????/???/??/???????????
 */
function shouldUseGroupedNav(items: DashboardNavItem[]): boolean {
  return items.some((it) => !!it.group);
}
...
export default function DashboardShell({
  ...
  const groupedMode = useMemo(() => shouldUseGroupedNav(navItems), [navItems]);
